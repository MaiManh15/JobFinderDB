...
NUser.post("/:account_id", async(req, res) => {
    ...
    const result = await conn.query(
        `INSERT INTO Normal_User( "NUser_id", account_id, "NUser_email", "NUser_phone_num", "NUser_name", age) 
...
NUser.get("/", async(req, res)=> {
    const db1 = JobFinderDb.getInstance();
    const conn = await db1.connect();
    const result = await conn.query(`SELECT * FROM Normal_User`);
...
NUser.get("/:NUser_id", async(req, res)=> {
    ...
    const result = await conn.query(`SELECT * FROM Normal_User where "NUser_id" = $1 `, [NUser_id]);
